...
import {
  type FreelancerCadreLang,
  getFreelancerCadreContent,
} from '../data/freelancerCadreContent';

const EspaceFreelancerCadrePage = () => {
  const navigate = useNavigate();

  const [lang, setLang] = useState<FreelancerCadreLang>('fr');
  ...
  const {
    ...
    backLabel,
    downloadPdfLabel,
  } = useMemo(() => getFreelancerCadreContent(lang), [lang]);

  const isArabic = lang === 'ar';
  const textAlignClass = isArabic ? 'text-right' : 'text-left';
  const sommaireButtonAlignClass = isArabic ? 'text-right' : 'text-left';

  const buildPrintableHtml = (printLang: FreelancerCadreLang) => {
    const content = getFreelancerCadreContent(printLang);
...
<html lang="${printLang}" dir="${dir}">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50" dir={isArabic ? 'rtl' : 'ltr'}>
            ...
            <div className="lg:sticky lg:top-24">
              <div className={`bg-white rounded-2xl border border-gray-200 shadow-sm p-6 ${textAlignClass}`}>
                ...
                <div className="mt-6 rounded-xl border border-blue-200 bg-blue-50 p-4">
                  <div className="text-sm font-semibold text-blue-900">{retainTitle}</div>
                  <p className="text-sm text-blue-900 mt-2">{retainText}</p>
                </div>
              </div>
            </div>
          </aside>
              ...
              {sections.map((s) => (
                <div key={s.id} id={s.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
                  <div className="flex items-start gap-3">
                    <div className="mt-1 h-8 w-8 rounded-lg bg-blue-600 text-white flex items-center justify-center text-sm font-bold">i</div>
                    <div>
...
export default EspaceFreelancerCadrePage;
